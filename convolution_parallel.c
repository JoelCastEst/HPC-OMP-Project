               ...
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    ...
    // Compute the kernel center position
    kCenterX = kernelSizeX / 2;
    kCenterY = kernelSizeY / 2;

    /*
     * The original version used moving pointers such as inPtr, inPtr2, outPtr and kPtr.
     * That approach is valid in serial code, but it carries state from one iteration to
     * the next one. For the OpenMP version, it is better to rewrite the computation using
     * explicit indices so that each output element can be computed independently.
     *
     * With this structure:
     * - input positions are accessed through (ii, jj)
     * - output positions are accessed through (i, j)
     * - kernel positions are accessed through (m, n)
     *
     * This removes the dependence on evolving pointers and makes the outer loop easier
     * to parallelize safely
     */

    // Traverse the output image row by row

    // Parallelize the outer loop over rows because each iteration computes
    // a different output row, reads shared input data, and writes to disjoint
    // output positions. This provides a safe OpenMP work decomposition with
    // very low synchronization requirements.
    // Static scheduling is used because all rows have similar computational cost,
    // so the work can be distributed evenly with low scheduling overhead.

    // Loop indices and the accumulation variable are private to each thread
    // to avoid interference between concurrent row computations.
    #pragma omp parallel for private(j,m,n,sum) schedule(static)
    for(i = 0; i < dataSizeY; ++i) {
        for(j = 0; j < dataSizeX; ++j) {
            sum = 0.0f;

            // Traverse all kernel rows
            for(m = 0; m < kernelSizeY; ++m) {
                int ii = i + (m - kCenterY);

                // Skip positions that fall outside the image boundaries
                if(ii < 0 || ii >= dataSizeY) continue;

                // Traverse all kernel columns
                for(n = 0; n < kernelSizeX; ++n) {
                    int jj = j + (n - kCenterX);

                    // Skip positions that fall outside the image boundaries
                    if(jj < 0 || jj >= dataSizeX) continue;

                    // Accumulate the weighted contribution of the valid neighbor pixel
                    sum += in[ii * dataSizeX + jj] * kernel[m * kernelSizeX + n];
                }
            }

            // Store the rounded result in the output image
            if(sum >= 0)
...
// Original sequential version kept for reference and comparison purposes.
// It was not used in the final OpenMP execution, but it helps document
// the original pointer-based implementation provided in the serial code.
int convolve2Dsequential(int* in, int* out, int dataSizeX, int dataSizeY,
               float* kernel, int kernelSizeX, int kernelSizeY)
{
    int i, j, m, n;
...
//    int headstored=0, imagestored=0, stored;
    ...
    // omp_get_wtime() is used instead of gettimeofday() because the assignment
    // explicitly requires this function for the OpenMP implementation.
    start = omp_get_wtime();
    tstart = start;
    kernelData kern=NULL;
    ...
    // The kernel size defines the halo used when the image is partitioned
    ...
    ////////////////////////////////////////
    // Reading image header and allocating image memory
    start = omp_get_wtime();
    ...
    // Duplicate the image structure for the output image
    start = omp_get_wtime();
    ...
    ////////////////////////////////////////
    // Initialize the output image file and store its header
    start = omp_get_wtime();
...
//            if (source->R[i]!=output->R[i] || source->G[i]!=output->G[i] || source->B[i]!=output->B[i]) printf("At position i=%d %d!=%d,%d!=%d,%d!=%d\n",i,source->R[i],output->R[i], source->G[i],output->G[i],source->B[i],output->B[i]);
        ...
        // The convolution is applied independently to the three color channels.
        // We considered parallelizing only RGB, but this would only create three
        // coarse tasks. We therefore selected row-level parallelism inside convolve2D()
        // as the main OpenMP strategy because it provides finer-grain parallel work.
        ...
        // CHUNK SAVING
        //////////////////////////////////////////////////////////////////////////////////////////////////
        // Store the resulting image partition
        start = omp_get_wtime();
